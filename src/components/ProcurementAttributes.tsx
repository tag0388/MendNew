  ...
  const reloadAttributes = useCallback(async () => {
    ...
    try {
      setAttributes(await fetchAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.procurementAttributes));
    } catch (error) {
      console.error('Error loading procurement attributes:', error);
    }
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await saveAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.procurementAttributes, updatedAttrs);
    } catch (error: any) {
