  ...
  const reloadAttributes = useCallback(async () => {
    ...
    try {
      setAttributes(await fetchAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.changeAttributes));
    } catch (error) {
      console.error('Error loading change attributes:', error);
    }
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await saveAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.changeAttributes, updatedAttrs);
    } catch (error: any) {
