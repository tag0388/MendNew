  ...
  const reloadAttributes = useCallback(async () => {
    ...
    try {
      setAttributes(await fetchAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.costCodeAttributes));
    } catch (error) {
      console.error('Error loading cost code attributes:', error);
    }
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await saveAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.costCodeAttributes, updatedAttrs);
    } catch (error: any) {
