  ...
  const reloadAttributes = useCallback(async () => {
    ...
    try {
      setAttributes(await fetchAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.progressAttributes));
    } catch (error) {
      console.error('Error loading progress attributes:', error);
    }
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await saveAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.progressAttributes, updatedAttrs);
    } catch (error: any) {
