  ...
  const reloadAttributes = useCallback(async () => {
    ...
    try {
      setAttributes(await fetchAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.lineItemAttributes));
    } catch (error) {
      console.error('Error loading line item attributes:', error);
    }
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await saveAttributeSet(
        project.enterpriseId, project.id, ATTRIBUTE_CATEGORY.lineItemAttributes, updatedAttrs);
    } catch (error: any) {
